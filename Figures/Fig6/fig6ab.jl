...
## Plot data and Greenspan model
    ...
    fig6a = @df confocal_all scatter(:R,:ϕ,:η,group=:InitialCondition,c = [colors[CellLine][d] for d ∈ [2500,5000,10000]],
            ms=4,
            size=(500,500),
            legend=:none,
    ...
    for (i,InitialCondition) ∈ enumerate(InitialConditions)
        ...
        plot!(fig6a,X[:,1],X[:,2],X[:,3],c = [colors[CellLine][d] for d ∈ [2500,5000,10000]][i])
...
## Subset data to perform ANOVA and test for dependence on initial condition
    ...
    # Subset
    confocal_all_1 = @subset(confocal_all,:η .> 0.0, :ϕ .> 0.7, :η .< 0.005 * :R .- 0.85)
    confocal_all_2 = @subset(confocal_all,(!).((:η .> 0.0) .& (:ϕ .> 0.7) .& (:η .< 0.005 * :R .- 0.85)))
    ...
    fig6b = plot([[x₀[i],x₁[i]] for i = 1:3]..., c=:black)

    @df confocal_all_1 scatter!(fig6b,:R,:ϕ,:η,group=:InitialCondition,c = [colors[CellLine][d] for d ∈ [2500,5000,10000]],
            ms=4,
            size=(500,500),
            legend=:none,
            xlabel="R",ylabel="phi",zlabel="eta";
            defaults_3D...)
    @df confocal_all_2 scatter!(fig6b,:R,:ϕ,:η,c=:black,α=0.2)

#########################################################################
## Save figure 6

fig6ab = plot(fig6a, fig6b, link=:all, size=(800,400),
        xlim=(-10.0,410.0), 
        ylim=(-0.02,1.02),
        zlim=(-0.02,1.02) 
)

#savefig("$(@__DIR__)/fig6ab.svg")