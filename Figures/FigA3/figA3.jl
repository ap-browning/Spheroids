#=
    ...
    Runtime: Approximately 30 seconds (after module compiled)
...
## Load and subset data
    ...
    confocal = @subset(CSV.read("Data/ConfocalData.csv",DataFrame),:CellLine .== CellLine, :Day .≥ 14.0)
...
## "Identity model": fit the mean of each observation at each time
    ...
    for (i,InitialCondition) ∈ enumerate(InitialConditions)
        ...
        for (j,Day) ∈ enumerate(Days1[i])
            ...
            if Σ[3,3] == 0.0
                # Log-likelihood
                local loglike = θ -> loglikelihood(MvNormal(θ[1:2],Σ[1:2,1:2]),confocal_subset[:,[:R,:ϕ]])
            else
                # Log-likelihood
...
#  outer radius at each time point (for parameter Q and R_crit)
    ...
    for (i,InitialCondition) ∈ enumerate(InitialConditions)
        ...
        for (j,Day) ∈ enumerate(Days2[i])
            ...
            # Fix for phase 2
            if Σ[3,3] == 0.0
                θmle2[i][j] = fill(NaN,3)
                CI2[i][j] = fill([NaN,NaN],3)
                continue
            end
...
for r = 1:2, c = 1:3, (i,InitialCondition) ∈ enumerate(InitialConditions)
    ...
    plot!(plts[r,c],
        ...
        xticks  = [14.0,16.0,18.0,21.0,24.0],
        xlabel  = "Time (d)",
        ylim    = [[[200,400],[0.0,1.0],[0.0,1.0]],[[0.6,1.0],[100.0,300.0],[0,20.0]]][r][c]
    )

end

figA3 = plot(plts[1,:]...,plts[2,:]...,legend=:none,size=(800,450))
#savefig(figA3,"$(@__DIR__)/figA3.svg")