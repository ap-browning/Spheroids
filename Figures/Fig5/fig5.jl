...
for v = 1:3
    for (i,InitialCondition) ∈ enumerate(InitialConditions)
        plot!(plts_profiles[v],Ψ[i][v],P[i][v] .- Lmle[i],
...
plts_CR = [plot([],[],[],xlim=(300.0,400.0),ylim=(0.85,0.95),zlim=(0.6,0.85),legend=:none)  # Observation means
           plot([],[],[],xlim=(0.6,0.95),ylim=(70.0,220.0),zlim=(0.2,1.3),legend=:none)]    # Greenspan model
...
fig5 = plot(plts_profiles...,plts_CR...,layout=@layout([grid(1,3); grid(1,2){0.6h}]),legend=:none,size=(700,550))
